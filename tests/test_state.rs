...
fn test_state_manager_new_with_nonexistent_file() {
    ...
    let manager = StateManager::new(state_file.clone()).expect("Failed to create state manager");
...
fn test_state_manager_update_and_retrieve() {
    ...
    let mut manager =
        StateManager::new(state_file.clone()).expect("Failed to create state manager");
    ...
    manager
        .update_resume_state(hash, output_path)
    ...
    let state = manager.get_resume_state(hash).expect("State should exist");
...
fn test_state_manager_persistence() {
    ...
    // Create manager and add some state
    {
        ...
        manager
            .update_resume_state("hash1", "/output1.json")
            .expect("Failed to update state");
        manager
            .update_resume_state("hash2", "/output2.json")
            .expect("Failed to update state");
    }

    // Create a new manager from the same file
    let manager = StateManager::new(state_file).expect("Failed to load state manager");
    ...
    // Verify output paths
    assert_eq!(
        manager.get_resume_state("hash1").unwrap().output_path,
        "/output1.json"
    );
    assert_eq!(
        manager.get_resume_state("hash2").unwrap().output_path,
        "/output2.json"
    );
}
...
fn test_state_manager_remove() {
    ...
    let mut manager = StateManager::new(state_file).expect("Failed to create state manager");
...
fn test_state_manager_clear_all() {
    ...
    manager
        ...
        .unwrap();
    manager
        .update_resume_state("hash3", "/output3.json")
        .unwrap();
